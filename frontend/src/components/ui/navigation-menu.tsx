...
function NavigationMenuLink({
  ...
  tabName,
  ...props
}: { tabName: string } & React.ComponentProps<
...
>) {
  ...
  return (
    <NavigationMenuPrimitive.Link
      data-active={tab === tabName}
      data-slot="navigation-menu-link"
      className={cn(
        "data-[active=true]:focus:bg-accent data-[active=true]:hover:bg-accent/80 data-[active=true]:bg-accent/60 data-[active=true]:text-accent-foreground hover:bg-accent/80 hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground ring-ring/10 dark:ring-ring/20 dark:outline-ring/40 outline-ring/50 [&_svg:not([class*='text-'])]:text-muted-foreground flex flex-col gap-1 rounded-sm p-2 text-sm transition-[color,box-shadow] focus-visible:ring-4 focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
